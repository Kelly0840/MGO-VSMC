...
# 2.Creation of RQ values in data frames                        # Change all to "...4"
# 2.1 GLO1

# 2.1.1 Duplicating dfs
avg_old_GLO1_df_4 <- avg_old_GLO1_dataframe[ , ]
avg_young_GLO1_df_4 <- avg_young_GLO1_dataframe[ , ]

# 2.1.2 Calculating RQ
# Young GLO1
# 2.1.2.1 Find average of No MGO values
no_mgo_avg_young_GLO1 <- mean(avg_young_GLO1_df_4[1:4,5], na.rm = TRUE)
# 2.1.2.2 Repeat this mean value in a column for non-paired normalisation
avg_young_GLO1_df_4$Mean.No.MGO.CT.GLO1 <- rep(no_mgo_avg_young_GLO1, length.out = nrow(avg_young_GLO1_df_4))
# 2.1.2.3 Calculating Delta Delta CT
avg_young_GLO1_df_4$Delta.Delta.CT.Young.GLO1 <- avg_young_GLO1_df_4[ , 5] - avg_young_GLO1_df_4[ ,6]
# 2.1.2.4 Calculating RQ
avg_young_GLO1_df_4$RQ.Young.GLO1.4 <- 2^(-avg_young_GLO1_df_4$Delta.Delta.CT.Young.GLO1)

# Old GLO1
# 2.1.2.1 Find average of No MGO values
# no_mgo_avg_old_GLO1 <- mean(avg_old_GLO1_df_3[1:4,5], na.rm = TRUE)
# 2.1.2.2 Repeat this mean value in a column for non-paired normalisation
avg_old_GLO1_df_4$Mean.No.MGO.CT.GLO1 <- rep(no_mgo_avg_young_GLO1, length.out = nrow(avg_old_GLO1_df_4)) # Change to young
# 2.1.2.3 Calculating Delta Delta CT
avg_old_GLO1_df_4$Delta.Delta.CT.Old.GLO1 <- avg_old_GLO1_df_4[ , 5] - avg_old_GLO1_df_4[ ,6]
# 2.1.2.4 Calculating RQ
avg_old_GLO1_df_4$RQ.Old.GLO1.4 <- 2^(-avg_old_GLO1_df_4$Delta.Delta.CT.Old.GLO1)
...
avg_young_ACTA2_df_4 <- rbind(avg_ori_young_ACTA2_dataframe, avg_new_young_ACTA2_dataframe)

# Arrange according to ascending concentration of MGO
avg_young_ACTA2_df_4$Sample.Name <- factor(avg_young_ACTA2_df_4$Sample.Name,
                                           levels = c("No MGO", "1μM", "10μM", "100μM"), ordered = TRUE)
avg_young_ACTA2_df_4 <- avg_young_ACTA2_df_4[order(avg_young_ACTA2_df_4$Sample.Name), ]

# 2.3.2 Calculating RQ
# 2.3.2.1 Find average of No MGO values
no_mgo_avg_young_ACTA2 <- mean(avg_young_ACTA2_df_4[1:4,5], na.rm = TRUE)
# 2.3.2.2 Repeat this mean value in a column for non-paired normalisation
avg_young_ACTA2_df_4$Mean.No.MGO.CT.ACTA2 <- rep(no_mgo_avg_young_ACTA2, length.out = nrow(avg_young_ACTA2_df_4))
# 2.3.2.3 Calculating Delta Delta CT
avg_young_ACTA2_df_4$Delta.Delta.CT.Young.ACTA2 <- avg_young_ACTA2_df_4[ , 5] - avg_young_ACTA2_df_4[ ,6]
# 2.3.2.4 Calculating RQ
avg_young_ACTA2_df_4$RQ.Young.ACTA2.4 <- 2^(-avg_young_ACTA2_df_4$Delta.Delta.CT.Young.ACTA2)
...
avg_old_ACTA2_df_4 <- rbind(avg_ori_old_ACTA2_dataframe, avg_new_old_ACTA2_dataframe)

# Arrange according to ascending concentration of MGO
avg_old_ACTA2_df_4$Sample.Name <- factor(avg_old_ACTA2_df_4$Sample.Name,
                                         levels = c("No MGO", "1μM", "10μM", "100μM"), ordered = TRUE)
avg_old_ACTA2_df_4 <- avg_old_ACTA2_df_4[order(avg_old_ACTA2_df_4$Sample.Name), ]
...
avg_old_ACTA2_df_4$Mean.No.MGO.CT.ACTA2 <- rep(no_mgo_avg_young_ACTA2, length.out = nrow(avg_old_ACTA2_df_4))
# 2.2.2.3 Calculating Delta Delta CT
avg_old_ACTA2_df_4$Delta.Delta.CT.Old.ACTA2 <- avg_old_ACTA2_df_4[ , 5] - avg_old_ACTA2_df_4[ ,6]
# 2.2.2.4 Calculating RQ
avg_old_ACTA2_df_4$RQ.Old.ACTA2.4 <- 2^(-avg_old_ACTA2_df_4$Delta.Delta.CT.Old.ACTA2)
...
# 2.4 Add "Age" column
#Old GLO1
avg_old_GLO1_df_4$Age <- c("Old")
avg_old_GLO1_df_4 <- avg_old_GLO1_df_4[ ,c("Sample.Name", "Sample.Number", "Group", "Age", "Average.C_", "Normalised.old.GLO1", "Mean.No.MGO.CT.GLO1",
                                           "Delta.Delta.CT.Old.GLO1", "RQ.Old.GLO1.4")]
#Young GLO1
avg_young_GLO1_df_4$Age <- c("Young")
avg_young_GLO1_df_4 <- avg_young_GLO1_df_4[ ,c("Sample.Name", "Sample.Number", "Group", "Age", "Average.C_", "Normalised.young.GLO1", "Mean.No.MGO.CT.GLO1",
                                               "Delta.Delta.CT.Young.GLO1", "RQ.Young.GLO1.4")]

#Old ACTA2
avg_old_ACTA2_df_4$Age <- c("Old")
avg_old_ACTA2_df_4 <- avg_old_ACTA2_df_4[ ,c("Sample.Name", "Sample.Number", "Group", "Age", "Average.C_", "Normalised.ACTA2", "Mean.No.MGO.CT.ACTA2",
                                             "Delta.Delta.CT.Old.ACTA2", "RQ.Old.ACTA2.4")]

#Young ACTA2
avg_young_ACTA2_df_4$Age <- c("Young")
avg_young_ACTA2_df_4 <- avg_young_ACTA2_df_4[ ,c("Sample.Name", "Sample.Number", "Group", "Age", "Average.C_", "Normalised.ACTA2", "Mean.No.MGO.CT.ACTA2",
                                                 "Delta.Delta.CT.Young.ACTA2", "RQ.Young.ACTA2.4")]


# 2.5 Combine old and young data frames

# 2.5.1 Renaming columns so they are the same in young and old, allow for rbind()
ACTA2_df4_colnames <-c("Sample.Name", "Sample.Number", "Group", "Age", "Average.C_", "Normalised.ACTA2", "Mean.No.MGO.CT.ACTA2",
                       "Delta.Delta.CT.ACTA2", "RQ.ACTA2")
GLO1_df4_colnames <-c("Sample.Name", "Sample.Number", "Group", "Age", "Average.C_", "Normalised.GLO1", "Mean.No.MGO.CT.GLO1",
                      "Delta.Delta.CT.GLO1", "RQ.GLO1")
# 2.5.2 Apply colnames to dfs
colnames(avg_old_ACTA2_df_4) <- ACTA2_df4_colnames
colnames(avg_young_ACTA2_df_4) <- ACTA2_df4_colnames
colnames(avg_old_GLO1_df_4) <- GLO1_df4_colnames
colnames(avg_young_GLO1_df_4) <- GLO1_df4_colnames
# 2.5.3 Combine dataframes
old_young_ACTA2_df_4 <- rbind(avg_young_ACTA2_df_4,avg_old_ACTA2_df_4)
old_young_GLO1_df_4 <- rbind(avg_young_GLO1_df_4,avg_old_GLO1_df_4)
...
RQ_young_GLO1_4 <- data.frame(Group = as.integer(names(avg_values)), delta.CT.young.GLO1 = as.vector(avg_values))
# 3.4.4 Add column for labels
RQ_young_GLO1_4$Sample <- c("No MGO", "1μM", "10μM", "100μM")
# 3.4.5 Rearrange columns
RQ_young_GLO1_4 <- RQ_young_GLO1_4[ , c("Sample", "Group", "delta.CT.young.GLO1")]
# 3.4.6 Add column for delta delta CT
RQ_young_GLO1_4$delta.delta.CT.young.GLO1 <- RQ_young_GLO1_4$delta.CT.young.GLO1 - RQ_young_GLO1_4[1, 3]
# 3.4.7 Rearrange columns
RQ_young_GLO1_4 <- RQ_young_GLO1_4[ , c("Sample", "Group", "delta.CT.young.GLO1", "delta.delta.CT.young.GLO1")]
# 3.4.8 Add column for RQ
RQ_young_GLO1_4$RQ.Young.GLO1 <- 2^(-RQ_young_GLO1_4$delta.delta.CT.young.GLO1)

print(RQ_young_GLO1_4)
...
RQ_old_GLO1_4 <- data.frame(Group = as.integer(names(avg_values)), delta.CT.old.GLO1 = as.vector(avg_values))
# 3.2.4 Add column for labels
RQ_old_GLO1_4$Sample <- c("No MGO", "1μM", "10μM", "100μM")
# 3.2.5 Rearrange columns
RQ_old_GLO1_4 <- RQ_old_GLO1_4[ , c("Sample", "Group", "delta.CT.old.GLO1")]
# 3.2.6 Add column for delta delta CT
RQ_old_GLO1_4$delta.delta.CT.old.GLO1 <- RQ_old_GLO1_4$delta.CT.old.GLO1 - RQ_young_GLO1_4[1, 3]             # Change to RQ_young_GLO1_4
# 3.2.7 Rearrange columns
RQ_old_GLO1_4 <- RQ_old_GLO1_4[ , c("Sample", "Group", "delta.CT.old.GLO1", "delta.delta.CT.old.GLO1")]
# 3.2.8 Add column for RQ
RQ_old_GLO1_4$RQ.Old.GLO1 <- 2^(-RQ_old_GLO1_4$delta.delta.CT.old.GLO1)
...
avg_young_ACTA2_dataframe <- avg_young_ACTA2_df_4[ , c(1,2,3,5,6)]
...
RQ_young_ACTA2_4 <- data.frame(Group = as.integer(names(avg_values)), delta.CT.young.ACTA2 = as.vector(avg_values))
# 3.8.4 Add column for labels
RQ_young_ACTA2_4$Sample <- c("No MGO", "1μM", "10μM", "100μM")
# 3.8.5 Rearrange columns
RQ_young_ACTA2_4 <- RQ_young_ACTA2_4[ , c("Sample", "Group", "delta.CT.young.ACTA2")]
# 3.8.6 Add column for delta delta CT
RQ_young_ACTA2_4$delta.delta.CT.young.ACTA2 <- RQ_young_ACTA2_4$delta.CT.young.ACTA2 - RQ_young_ACTA2_4[1, 3]
# 3.8.7 Rearrange columns
RQ_young_ACTA2_4 <- RQ_young_ACTA2_4[ , c("Sample", "Group", "delta.CT.young.ACTA2", "delta.delta.CT.young.ACTA2")]
# 3.8.8 Add column for RQ
RQ_young_ACTA2_4$RQ.Young.ACTA2 <- 2^(-RQ_young_ACTA2_4$delta.delta.CT.young.ACTA2)

print(RQ_young_ACTA2_4)
...
avg_old_ACTA2_dataframe <- avg_old_ACTA2_df_4[ , c(1,2,3,5,6)]
...
RQ_old_ACTA2_4 <- data.frame(Group = as.integer(names(avg_values)), delta.CT.old.ACTA2 = as.vector(avg_values))
# 3.6.4 Add column for labels
RQ_old_ACTA2_4$Sample <- c("No MGO", "1μM", "10μM", "100μM")
# 3.6.5 Rearrange columns
RQ_old_ACTA2_4 <- RQ_old_ACTA2_4[ , c("Sample", "Group", "delta.CT.old.ACTA2")]
# 3.6.6 Add column for delta delta CT
RQ_old_ACTA2_4$delta.delta.CT.old.ACTA2 <- RQ_old_ACTA2_4$delta.CT.old.ACTA2 - RQ_young_ACTA2_4[1, 3]             # Change to RQ_young_ACTA2_4
# 3.6.7 Rearrange columns
RQ_old_ACTA2_4 <- RQ_old_ACTA2_4[ , c("Sample", "Group", "delta.CT.old.ACTA2", "delta.delta.CT.old.ACTA2")]
# 3.6.8 Add column for RQ
RQ_old_ACTA2_4$RQ.Old.ACTA2 <- 2^(-RQ_old_ACTA2_4$delta.delta.CT.old.ACTA2)

print(RQ_old_ACTA2_4)
...
# 4.1 Add "Age" columns
# Old GLO1
RQ_old_GLO1_4$Age <- c("Old")
RQ_old_GLO1_4 <- RQ_old_GLO1_4[ ,c("Sample", "Group", "Age", "delta.CT.old.GLO1", "delta.delta.CT.old.GLO1", "RQ.Old.GLO1")]
# Young GLO1
RQ_young_GLO1_4$Age <- c("Young")
RQ_young_GLO1_4 <- RQ_young_GLO1_4[ ,c("Sample", "Group", "Age", "delta.CT.young.GLO1", "delta.delta.CT.young.GLO1", "RQ.Young.GLO1")]
# Old ACTA2
RQ_old_ACTA2_4$Age <- c("Old")
RQ_old_ACTA2_4 <- RQ_old_ACTA2_4[ ,c("Sample", "Group", "Age", "delta.CT.old.ACTA2", "delta.delta.CT.old.ACTA2", "RQ.Old.ACTA2")]
# Young ACTA2
RQ_young_ACTA2_4$Age <- c("Young")
RQ_young_ACTA2_4 <- RQ_young_ACTA2_4[ ,c("Sample", "Group", "Age", "delta.CT.young.ACTA2", "delta.delta.CT.young.ACTA2", "RQ.Young.ACTA2")]
...
colnames(RQ_old_GLO1_4) <- GLO1_meanRQ_colnames
colnames(RQ_young_GLO1_4) <- GLO1_meanRQ_colnames
colnames(RQ_old_ACTA2_4) <- ACTA2_meanRQ_colnames
colnames(RQ_young_ACTA2_4) <- ACTA2_meanRQ_colnames


#4.4 Combine dataframes
Mean_RQ_GLO1_4 <- rbind(RQ_young_GLO1_4,RQ_old_GLO1_4)
Mean_RQ_ACTA2_4 <- rbind(RQ_young_ACTA2_4,RQ_old_ACTA2_4)
...
# 5.1 GLO1
summary_stats <- old_young_GLO1_df_4 %>%
...
old_young_GLO1_df_4$Sample.Name <- factor(old_young_GLO1_df_4$Sample.Name, levels = levels_order)
summary_stats$Sample.Name <- factor(summary_stats$Sample.Name, levels = levels_order)
Mean_RQ_GLO1_4$Sample <- factor(Mean_RQ_GLO1_4$Sample, levels = levels_order)

GLO1_bsplot_4 <- ggplot(old_young_GLO1_df_4, aes(x = Sample.Name, y = RQ.GLO1, colour = Age)) +
  ...
  geom_point(data = Mean_RQ_GLO1_4,
  ...
  labs(title = "Effect of MGO Concentration and Age on GLO1 Expression",
       x = "MGO Concentration", y = "RQ")


# 5.2 ACTA2
summary_stats <- old_young_ACTA2_df_4 %>%
  group_by(Sample.Name, Age) %>%
  summarise(
...
levels_order <- c("No MGO", "1μM", "10μM", "100μM")
old_young_ACTA2_df_4$Sample.Name <- factor(old_young_ACTA2_df_4$Sample.Name, levels = levels_order)
summary_stats$Sample.Name <- factor(summary_stats$Sample.Name, levels = levels_order)
Mean_RQ_ACTA2_4$Sample <- factor(Mean_RQ_ACTA2_4$Sample, levels = levels_order)

ACTA2_bsplot_4 <- ggplot(old_young_ACTA2_df_4, aes(x = Sample.Name, y = RQ.ACTA2, colour = Age)) +
  geom_beeswarm(size = 2, cex = 3, dodge.width = 0.6) +
  geom_errorbar(data = summary_stats, inherit.aes = FALSE,
                aes(x = Sample.Name, ymin = mean_RQ - se, ymax = mean_RQ + se, colour = Age),
                width = 0.2, size = 0.7,
                position = position_dodge(width = 0.6)) +
  geom_point(data = Mean_RQ_ACTA2_4,
  ...
  labs(title = "Effect of MGO Concentration and Age on ACTA2 Expression",
       x = "MGO Concentration", y = "RQ")
...
# 6.1 GLO1
# 6.1.1 Run analysis
GLO1_two_way_ANOVA_4 <- aov(data = old_young_GLO1_df_4, Normalised.GLO1 ~ Age * Sample.Name)
# 6.1.2 View ANOVA table
summary(GLO1_two_way_ANOVA_4)
# 6.1.3 Convert to dataframe
GLO1_two_way_ANOVA_df_4 <- as.data.frame(summary(GLO1_two_way_ANOVA_4)[[1]])

# 6.2 ACTA2
# 6.2.1 Run analysis
ACTA2_two_way_ANOVA_4 <- aov(data = old_young_ACTA2_df_4, Normalised.ACTA2 ~ Age * Sample.Name)
# 6.2.2 View ANOVA table
summary(ACTA2_two_way_ANOVA_4)
# 6.2.3 Convert to dataframe
ACTA2_two_way_ANOVA_df_4 <- as.data.frame(summary(ACTA2_two_way_ANOVA_4)[[1]])
...
Tukey_results_GLO1_4 <- TukeyHSD(GLO1_two_way_ANOVA_4)

# 7.1.2 Extract interaction term results and convert to dataframe, adding "comparison" column
Tukey_results_GLO1_df_4 <- as.data.frame(Tukey_results_GLO1_4$`Age:Sample.Name`) %>%
...
p_GLO1_matrix_4 <- matrix(NA, nrow = length(group_order), ncol = length(group_order),
                        dimnames = list(group_order, group_order))

# 7.1.5 P values in lower diagonal
for (i in seq_len(nrow(Tukey_results_GLO1_df_4))) {
  comps <- unlist(strsplit(Tukey_results_GLO1_df_4$Comparison[i], "-"))
  ...
  if (all(comps %in% group_order)) {
    ...
    if (!is.na(idx1) && !is.na(idx2) && idx1 != idx2) {
      lower <- max(idx1, idx2)
      upper <- min(idx1, idx2)
      p_GLO1_matrix_4[lower, upper] <- sprintf('%.5f', Tukey_results_GLO1_df_4$`p adj`[i])
    }
  }
}

# 7.1.6 Filling diagonal with dashes
diag(p_GLO1_matrix_4) <- "--"

# 7.1.7 Converting to data frame
p_GLO1_table_4 <- as.data.frame(p_GLO1_matrix_4)
print(p_GLO1_table_4, na.print = "")



# 7.2 ACTA2
# 7.2.1 Run Tukey HSD post hoc test on two-way ANOVA results
Tukey_results_ACTA2_4 <- TukeyHSD(ACTA2_two_way_ANOVA_4)

# 7.2.2 Extract interaction term results and convert to dataframe, adding "comparison" column
Tukey_results_ACTA2_df_4 <- as.data.frame(Tukey_results_ACTA2_4$`Age:Sample.Name`) %>%
  tibble::rownames_to_column("Comparison")
...
p_ACTA2_matrix_4 <- matrix(NA, nrow = length(group_order), ncol = length(group_order),
                         dimnames = list(group_order, group_order))

# 7.2.5 P values in lower diagonal
for (i in seq_len(nrow(Tukey_results_ACTA2_df_4))) {
  comps <- unlist(strsplit(Tukey_results_ACTA2_df_4$Comparison[i], "-"))
  ...
  if (all(comps %in% group_order)) {
    ...
    if (!is.na(idx1) && !is.na(idx2) && idx1 != idx2) {
      lower <- max(idx1, idx2)
      upper <- min(idx1, idx2)
      p_ACTA2_matrix_4[lower, upper] <- sprintf('%.5f', Tukey_results_ACTA2_df_4$`p adj`[i])
    }
  }
}

# 7.2.6 Filling diagonal with dashes
diag(p_ACTA2_matrix_4) <- "--"

# 7.2.7 Converting to data frame
p_ACTA2_table_4 <- as.data.frame(p_ACTA2_matrix_4)
print(p_ACTA2_table_4, na.print = "")
...
# ACTA2
ggsave("ACTA2_bsplot_small_4.pdf", plot = ACTA2_bsplot_4, width = 21, height = 13, units = "cm")
ggsave("ACTA2_bsplot_big_4.pdf", plot = ACTA2_bsplot_4, width = 32, height = 18, units = "cm")
# GLO1
ggsave("GLO1_bsplot_small_4.pdf", plot = GLO1_bsplot_4, width = 21, height = 13, units = "cm")
ggsave("GLO1_bsplot_big_4.pdf", plot = GLO1_bsplot_4, width = 32, height = 18, units = "cm")


# 8.2 Saving Tukey results

# 8.2.2 Creating new workbook
wb_GLO1_ACTA2_4 <- createWorkbook()

# 8.2.3 Processing workbook data
process_and_write_table(wb_GLO1_ACTA2_4, "ACTA2", p_ACTA2_table_4)
process_and_write_table(wb_GLO1_ACTA2_4, "GLO1", p_GLO1_table_4)

# 8.2.4 Saving workbook
saveWorkbook(wb_GLO1_ACTA2_4, file = "Tukey_Table_GLO1_ACTA2_4.xlsx", overwrite = TRUE)
