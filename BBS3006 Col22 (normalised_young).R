...
# Young data
# 2.1 Duplicating dfs
avg_young_Col22_df_4 <- avg_young_Col22_dataframe[ , ]


# 2.2 Calculating RQ
# Young Col22
# 2.2.1 Find average of No MGO values
no_mgo_avg_young_Col22 <- mean(avg_young_Col22_df_4[1:4,5], na.rm = TRUE)
# 2.2.2 Repeat this mean value in a column for non-paired normalisation
avg_young_Col22_df_4$Mean.No.MGO.CT.Col22 <- rep(no_mgo_avg_young_Col22, length.out = nrow(avg_young_Col22_df_4))
# 2.2.3 Calculating Delta Delta CT
avg_young_Col22_df_4$Delta.Delta.CT.Young.Col22 <- avg_young_Col22_df_4[ , 5] - avg_young_Col22_df_4[ ,6]
# 2.2.4 Calculating RQ
avg_young_Col22_df_4$RQ.Young.Col22.4 <- 2^(-avg_young_Col22_df_4$Delta.Delta.CT.Young.Col22)


# Old data
# 2.1 Duplicating dfs
avg_old_Col22_df_4 <- avg_old_Col22_dataframe[ , ]


# 2.2 Calculating RQ
# Old Col22
# 2.2.1 Find average of No MGO values
#no_mgo_avg_old_Col22 <- mean(avg_old_Col22_df_3[1:4,5], na.rm = TRUE)            # Change to young
# 2.2.2 Repeat this mean value in a column for non-paired normalisation
avg_old_Col22_df_4$Mean.No.MGO.CT.Col22 <- rep(no_mgo_avg_young_Col22, length.out = nrow(avg_old_Col22_df_4))
# 2.2.3 Calculating Delta Delta CT
avg_old_Col22_df_4$Delta.Delta.CT.Old.Col22 <- avg_old_Col22_df_4[ , 5] - avg_old_Col22_df_4[ ,6]
# 2.2.4 Calculating RQ
avg_old_Col22_df_4$RQ.Old.Col22.4 <- 2^(-avg_old_Col22_df_4$Delta.Delta.CT.Old.Col22)
...
# 2.3 Add "Age" column
#Old Col22
avg_old_Col22_df_4$Age <- c("Old")
avg_old_Col22_df_4 <- avg_old_Col22_df_4[ ,c("Sample.Name", "Sample.Number", "Group", "Age", "Average.C_", "Normalised.old.Col22", "Mean.No.MGO.CT.Col22",
                                             "Delta.Delta.CT.Old.Col22", "RQ.Old.Col22.4")]
#Young Col22
avg_young_Col22_df_4$Age <- c("Young")
avg_young_Col22_df_4 <- avg_young_Col22_df_4[ ,c("Sample.Name", "Sample.Number", "Group", "Age", "Average.C_", "Normalised.young.Col22", "Mean.No.MGO.CT.Col22",
                                                 "Delta.Delta.CT.Young.Col22", "RQ.Young.Col22.4")]


# 2.4 Combine old and young data frames

# 2.4.1 Renaming columns so they are the same in young and old, allow for rbind()
Col22_df4_colnames <-c("Sample.Name", "Sample.Number", "Group", "Age", "Average.C_", "Normalised.Col22", "Mean.No.MGO.CT.Col22",
                       "Delta.Delta.CT.Col22", "RQ.Col22")
# 2.4.2 Apply colnames to dfs
colnames(avg_old_Col22_df_4) <- Col22_df4_colnames
colnames(avg_young_Col22_df_4) <- Col22_df4_colnames
# 2.4.3 Combine dataframes
old_young_Col22_df_4 <- rbind(avg_young_Col22_df_4,avg_old_Col22_df_4)
...
RQ_young_Col22_4 <- data.frame(Group = as.integer(names(avg_values)), delta.CT.young.Col22 = as.vector(avg_values))
# 3.4.4 Add column for labels
RQ_young_Col22_4$Sample <- c("No MGO", "1μM", "10μM", "100μM")
# 3.4.5 Rearrange columns
RQ_young_Col22_4 <- RQ_young_Col22_4[ , c("Sample", "Group", "delta.CT.young.Col22")]
# 3.4.6 Add column for delta delta CT
RQ_young_Col22_4$delta.delta.CT.young.Col22 <- RQ_young_Col22_4$delta.CT.young.Col22 - RQ_young_Col22_4[1, 3]
# 3.4.7 Rearrange columns
RQ_young_Col22_4 <- RQ_young_Col22_4[ , c("Sample", "Group", "delta.CT.young.Col22", "delta.delta.CT.young.Col22")]
# 3.4.8 Add column for RQ
RQ_young_Col22_4$RQ.Young.Col22 <- 2^(-RQ_young_Col22_4$delta.delta.CT.young.Col22)

print(RQ_young_Col22_4)
...
RQ_old_Col22_4 <- data.frame(Group = as.integer(names(avg_values)), delta.CT.old.Col22 = as.vector(avg_values))
# 3.2.4 Add column for labels
RQ_old_Col22_4$Sample <- c("No MGO", "1μM", "10μM", "100μM")
# 3.2.5 Rearrange columns
RQ_old_Col22_4 <- RQ_old_Col22_4[ , c("Sample", "Group", "delta.CT.old.Col22")]
# 3.2.6 Add column for delta delta CT
RQ_old_Col22_4$delta.delta.CT.old.Col22 <- RQ_old_Col22_4$delta.CT.old.Col22 - RQ_young_Col22_4[1, 3]             # Change to RQ_young_Col22_4
# 3.2.7 Rearrange columns
RQ_old_Col22_4 <- RQ_old_Col22_4[ , c("Sample", "Group", "delta.CT.old.Col22", "delta.delta.CT.old.Col22")]
# 3.2.8 Add column for RQ
RQ_old_Col22_4$RQ.Old.Col22 <- 2^(-RQ_old_Col22_4$delta.delta.CT.old.Col22)

print(RQ_old_Col22_4)
...
# 4.1 Add "Age" columns
# Old Col22
RQ_old_Col22_4$Age <- c("Old")
RQ_old_Col22_4 <- RQ_old_Col22_4[ ,c("Sample", "Group", "Age", "delta.CT.old.Col22", "delta.delta.CT.old.Col22", "RQ.Old.Col22")]
# Young Col22
RQ_young_Col22_4$Age <- c("Young")
RQ_young_Col22_4 <- RQ_young_Col22_4[ ,c("Sample", "Group", "Age", "delta.CT.young.Col22", "delta.delta.CT.young.Col22", "RQ.Young.Col22")]
...
colnames(RQ_old_Col22_4) <- Col22_meanRQ_colnames
colnames(RQ_young_Col22_4) <- Col22_meanRQ_colnames


# 4.4 Combine dataframes
Mean_RQ_Col22_4 <- rbind(RQ_young_Col22_4,RQ_old_Col22_4)
...
# 5.1 Col22
summary_stats <- old_young_Col22_df_4 %>%
...
old_young_Col22_df_4$Sample.Name <- factor(old_young_Col22_df_4$Sample.Name, levels = levels_order)
summary_stats$Sample.Name <- factor(summary_stats$Sample.Name, levels = levels_order)
Mean_RQ_Col22_4$Sample <- factor(Mean_RQ_Col22_4$Sample, levels = levels_order)

Col22_bsplot_4 <- ggplot(old_young_Col22_df_4, aes(x = Sample.Name, y = RQ.Col22, colour = Age)) +
  ...
  geom_point(data = Mean_RQ_Col22_4,
  ...
  labs(title = "Effect of MGO Concentration and Age on COL22A1 Expression",
...
# 6.1 Col22
# 6.1.1 Run analysis
Col22_two_way_ANOVA_4 <- aov(data = old_young_Col22_df_4, Normalised.Col22 ~ Age * Sample.Name)
# 6.1.2 View ANOVA table
summary(Col22_two_way_ANOVA_4)
# 6.1.3 Convert to dataframe
Col22_two_way_ANOVA_df_4 <- as.data.frame(summary(Col22_two_way_ANOVA_4)[[1]])
...
Tukey_results_Col22_4 <- TukeyHSD(Col22_two_way_ANOVA_4)

# 7.1.2 Extract interaction term results and convert to dataframe, adding "comparison" column
Tukey_results_Col22_df_4 <- as.data.frame(Tukey_results_Col22_4$`Age:Sample.Name`) %>%
...
p_Col22_matrix_4 <- matrix(NA, nrow = length(group_order), ncol = length(group_order),
                         dimnames = list(group_order, group_order))

# 7.1.5 P values in lower diagonal
for (i in seq_len(nrow(Tukey_results_Col22_df_4))) {
  comps <- unlist(strsplit(Tukey_results_Col22_df_4$Comparison[i], "-"))
  ...
  if (all(comps %in% group_order)) {
    ...
    if (!is.na(idx1) && !is.na(idx2) && idx1 != idx2) {
      lower <- max(idx1, idx2)
      upper <- min(idx1, idx2)
      p_Col22_matrix_4[lower, upper] <- sprintf('%.5f', Tukey_results_Col22_df_4$`p adj`[i])
    }
  }
}

# 7.1.6 Filling diagonal with dashes
diag(p_Col22_matrix_4) <- "--"

# 7.1.7 Converting to data frame
p_Col22_table_4 <- as.data.frame(p_Col22_matrix_4)
print(p_Col22_table_4, na.print = "")
...
# Col22
ggsave("Col22_bsplot_small_4.pdf", plot = Col22_bsplot_4, width = 21, height = 13, units = "cm")
ggsave("Col22_bsplot_big_4.pdf", plot = Col22_bsplot_4, width = 32, height = 18, units = "cm")


# 8.2 Saving Tukey results

# 8.2.2 Creating new workbook
wb_Col22_4 <- createWorkbook()

# 8.2.3 Processing workbook data
process_and_write_table(wb_Col22_4, "Col22", p_Col22_table_4)

# 8.2.4 Saving workbook
saveWorkbook(wb_Col22_4, file = "Tukey_Table_Col22_4.xlsx", overwrite = TRUE)
